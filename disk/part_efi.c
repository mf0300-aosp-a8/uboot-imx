		...
		lbaint_t lastlba)
{
	...
	if (le64_to_cpu(gpt_h->signature) != GPT_HEADER_SIGNATURE) {
		printf("signature is wrong: 0x%llX != 0x%llX\n",
...
int part_get_info_efi(struct blk_desc *dev_desc, int part, disk_partition_t *info)
{
	...
	uuid_bin_to_str(gpt_pte[part - 1].unique_partition_guid.b, 
		info->uuid, UUID_STR_FORMAT_GUID);
#endif
...
#ifdef DEBUG_PRINT_DETAILED_EFI
...
#endif
		"\n",
		__func__, part, info->blksz, info->start, info->size,
		info->bootable, info->sys_ind,
		info->name
#if CONFIG_IS_ENABLED(PARTITION_UUIDS)
		,info->uuid
#endif
#if CONFIG_PARTITION_TYPE_GUID
		,info->type_guid
#endif
		);
#endif // DEBUG_PRINT_DETAILED_EFI
