...
void gen_info(bitmap_t *b, uint16_t n_colors)
{
	printf("/*\n"
		...
		n_colors <= 240 ? DEFAULT_CMAP_SIZE : 0);
}
...
int main (int argc, char *argv[])
{
	...
	bitmap_t bmp;
	bitmap_t *b = &bmp;
	uint16_t data_offset, n_colors, n_colors_readed;
	...
	skip_bytes (fp, 22);
	if (fread (&n_colors_readed, sizeof (uint16_t), 1, fp) != 1) {
		// sometimes it's incorrectly calculated
	...
	n_colors = le_short(n_colors_readed);
	...
	printf("/*\n"
		...
		" *\n"
		" * DO NOT EDIT\n"
		" * Detected palette with n_colors=%d (n_colors_readed=%d) colors\n"
		" * Detected width=%d height=%d data offset=0x%p */\n\n\n"
		"#ifndef __BMP_LOGO_DATA_H__\n"
		"#define __BMP_LOGO_DATA_H__\n\n", 
		n_colors, n_colors_readed, b->width, b->height, data_offset);
